...
func NewCloudRunDeployer(runCtx *runcontext.RunContext) *CloudRunDeployer {
	return &CloudRunDeployer{
		...
		CloudRun: deploy.CLI{
			CLI:         CloudRun.NewFromRunContext(runCtx),
			Flags:       runCtx.Cfg.Deploy.CloudRunDeploy.Flags,
...
func (k *CloudRunDeployer) Dependencies() ([]string, error) {
	return k.manifestFiles(k.CloudRunDeploy.Manifests)
}

func (k *CloudRunDeployer) manifestFiles(manifests []string) ([]string, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "expanding CloudRun manifest paths")
	}
...
func (k *CloudRunDeployer) readManifests(ctx context.Context) (deploy.ManifestList, error) {
	// Get file manifests
	...
	for _, manifest := range k.CloudRunDeploy.Manifests {
		if util.IsURL(manifest) {
	...
	return k.CloudRun.ReadManifests(ctx, manifests)
}
...
func (k *CloudRunDeployer) readRemoteManifest(ctx context.Context, name string) ([]byte, error) {
	var args []string
	ns := ""
	...
	err := k.CloudRun.RunInNamespace(ctx, nil, &manifest, "get", ns, args...)
	if err != nil {
...
func (k *CloudRunDeployer) renderManifests(ctx context.Context, out io.Writer, builds []build.Artifact) (deploy.ManifestList, error) {
	if err := k.CloudRun.CheckVersion(ctx); err != nil {
		color.Default.Fprintln(out, "CloudRun client version:", k.CloudRun.Version(ctx))
